...
class UnetCrossValidation:
    ...
    def train_cross_validation(self, results_path: str = "models\\unet\\results"):
    ...
    def resume_fold_from_checkpoint(self, fold_num, checkpoint_path, results_path):
        ...
        # Reconstruct the split using the same random seed
        from sklearn.model_selection import KFold
...
if __name__ == "__main__":
    unet_cv = UnetCrossValidation(
        learning_rate=1e-4,
        num_workers=8,
        prefetch_factor=None,
        gradient_accumulation_steps=4,
        cudnn_checkpointing=True,
        epochs=200,                        # Epochs per fold
        n_folds=5,                         # 5-fold cross-validation
        early_stopping_patience=0)         # Stop if no improvement 
    
    # Run 5-fold cross-validation training
    results_base = "models\\unet\\results\\5fold_cv_AdamW_200epochs_no_early_stopping"
    fold_models, fold_results = unet_cv.train_cross_validation(results_path=results_base)
    
    # Test with ensemble of all fold models
    unet_cv.ensemble_test(results_path=results_base)
